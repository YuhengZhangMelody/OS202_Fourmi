...
class Colony:
    ...
    def __init__(self, nb_ants, pos_init, max_life):
        ...
        # self.sprites = []  # List of sprites for the ants
        ...
        img = pg.image.load("ants.png").convert_alpha()
        for i in range(0, 32, 8):
            sprite = pg.Surface.subsurface(img, i, 0, 8, 8)
            sprite_string = pg.image.tostring(sprite, 'RGBA')
            size = sprite.get_size()
            self.sprites.append((sprite_string, size))


    def deserialze_sprites(self):
        sprites = []
        for sprite_string, size in self.sprites:
            sprite = pg.image.fromstring(sprite_string, size, 'RGBA')
    ...
    def display(self, screen):
        sprites = self.deserialze_sprites()
    ...
    def merge(self, other):
        self.seeds = np.concatenate((self.seeds, other.seeds))
        self.is_loaded = np.concatenate((self.is_loaded, other.is_loaded))
        self.max_life = np.concatenate((self.max_life, other.max_life))
        self.age = np.concatenate((self.age, other.age))
        self.historic_path = np.concatenate((self.historic_path, other.historic_path))
        self.directions = np.concatenate((self.directions, other.directions))
        self.sprites = self.sprites + other.sprites

    def show_info(self):
        print(f"Seeds : {self.seeds.shape}")
        print(f"Is loaded : {self.is_loaded.shape}")
        print(f"Max life : {self.max_life.shape}")
        print(f"Age : {self.age.shape}")
        print(f"Historic path : {self.historic_path.shape}")
        print(f"Directions : {self.directions.shape}")

    # def split(self, nb_ants):
    #     other = Colony(0, (0, 0), 0)
    #     other.seeds = self.seeds[nb_ants:]
    #     other.is_loaded = self.is_loaded[nb_ants:]
    #     other.max_life = self.max_life[nb_ants:]
    #     other.age = self.age[nb_ants:]
    #     other.historic_path = self.historic_path[nb_ants:]
    #     other.directions = self.directions[nb_ants:]
    #     other.sprites = self.sprites[nb_ants:]
    #     self.seeds = self.seeds[:nb_ants]
    #     self.is_loaded = self.is_loaded[:nb_ants]
    #     self.max_life = self.max_life[:nb_ants]
    #     self.age = self.age[:nb_ants]
    #     self.historic_path = self.historic_path[:nb_ants]
    #     self.directions = self.directions[:nb_ants]
    #     self.sprites = self.sprites[:nb_ants]
    #     return other
                

def global_update_pherom_avg(pherom_gather):
    pherom_update = np.zeros(pherom_gather[0].shape, dtype=np.double)
    pherom_globe = np.zeros(pherom_gather[0].shape, dtype=np.double)
    for phenom in pherom_gather:
        for i in range(0, phenom.shape[0]):
            for j in range(0, phenom.shape[1]):
                # if phenom[i, j] > 0:
                pherom_globe[i, j] += phenom[i, j]
                if phenom[i, j] > 0:
                    pherom_update[i, j] += 1
    pherom_update[pherom_update == 0] = 1
    pherom_globe = pherom_globe / pherom_update
    return pherom_globe
...
if __name__ == "__main__":
    ...
    size = comm.Get_size()
    if size <= 2:
        print("This program must be run with more than 2 processes")
        exit(0)
    ...
    nb_ants_proc = nb_ants // (size-1)
    nb_ants_rest = nb_ants % (size-1)
    max_life = 500
    if len(sys.argv) > 3:
    ...
    if rank == 0:
        ...
        # ants = Colony(nb_ants, pos_nest, max_life)
        # ants_globe = None
        pherom_globe = pheromone.Pheromon(size_laby, pos_food, alpha, beta)
        pherom_buffer = np.empty((size_laby[0]+2, size_laby[1]+2), dtype=np.double)
        # pherom_gather = []
        # ants_gather = []
        mazeImg = a_maze.display()
        food_counter = 0
        ...
        # a_maze = None
        ants = Colony(nb_ants_proc+nb_ants_rest, pos_nest, max_life)
        pherom = pheromone.Pheromon(size_laby, pos_food, alpha, beta)
        pherom_buffer = np.empty((size_laby[0]+2, size_laby[1]+2), dtype=np.double)
        # pherom_gather = np.empty((size_laby[0]+2, size_laby[1]+2), dtype=np.double)
        mazeImg = None
        food_counter = 0
    else:
        ants = Colony(nb_ants_proc, pos_nest, max_life)
        pherom = pheromone.Pheromon(size_laby, pos_food, alpha, beta)
        pherom_buffer = np.empty((size_laby[0]+2, size_laby[1]+2), dtype=np.double)
        # pherom_gather = np.empty((size_laby[0]+2, size_laby[1]+2), dtype=np.double)
        mazeImg = None
    ...
    while True:
        ...
        if rank == 0:
            ...
            for i in range(1, size):
                comm.send(pherom_globe.pheromon, dest=i, tag=i*10)
            ...
            # print(sum(pherom_gather).shape)
            # ants_globe.show_info()
            pherom_globe.pheromon = global_update_pherom_avg(pherom_gather)
            # pherom_buffer = np.sum(pherom_gather, axis=0)
            # pherom_buffer[pherom_buffer > 1] = 1
            # pherom_globe.pheromon = pherom_buffer
            ...
            pg.display.update()
            # end = time.time()
            if food_counter != 0 and not snapshop_taken:
            ...
            # pherom_buffer = pherom.pheromon
            comm.send(pherom.pheromon, dest=0, tag=rank*10+1)
            comm.send(ants, dest=0, tag=rank*10+2)
            comm.send(food_counter, dest=0, tag=rank*10+3)
            # pherom_gather = comm.gather(pherom_buffer, root=0)
            # print(pherom_gather.shape)
                # food_counter = comm.reduce(food_counter, op=MPI.SUM, root=0)
                # comm.gather(food_counter, root=0)


    if rank == 0:
        end = time.time()
        output = f"FPS : {count/(end-deb):6.2f}, nourriture : {food_counter:7d}"
        with open("output_proc.txt", "w") as file:
            file.write(output)
