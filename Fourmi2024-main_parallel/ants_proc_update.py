...
def global_update_pherom(pherom_gather):
...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        pherom_buffer = pherom_globe.pheromon
        pherom_prev = pherom_globe.pheromon
        pherom_curr = pherom_globe.pheromon
        # print(pherom_)
        # pherom_gather = []
        # ants_gather = []
        mazeImg = a_maze.display()
        ...
        # a_maze = None
        ants_loc = Colony(nb_ants_proc+nb_ants_rest, pos_nest, max_life)
        pherom_globe = pheromone.Pheromon(size_laby, pos_food, alpha, beta)
        # pherom = pherom_globe.pheromon
        pherom_prev = pherom_globe.pheromon
        pherom_curr = pherom_globe.pheromon
        pherom_buffer = pherom_globe.pheromon
        ...
        ants_loc = Colony(nb_ants_proc, pos_nest, max_life)
        pherom_globe = pheromone.Pheromon(size_laby, pos_food, alpha, beta)
        # pherom = pheromone.Pheromon(size_laby, pos_food, alpha, beta)
        pherom_prev = pherom_globe.pheromon
        pherom_curr = pherom_globe.pheromon
    ...
    while True:
        ...
        for i in range(1, size):
            if rank == i and i > 1:
                # print(f"Process {rank} recieving")
        ...
        for i in range(1, size):
            if rank == i:
                # comm.send(food_counter_curr, dest=0, tag=rank*100+1)
                comm.send(food_counter, dest=0, tag=rank*100+1)
                comm.send(ants_loc, dest=0, tag=rank*100+2)
            if rank == 1:
                comm.send(pherom_globe.pheromon, dest=0, tag=rank*100+3)
        ...
        if rank == 0:
            ...
            for j in range(1, size):
                # food_counter_list.append( comm.recv(source=j, tag=j*100+1))
                food_counter += comm.recv(source=j, tag=j*100+1)
                ants_tmp = comm.recv(source=j, tag=j*100+2)
                ants_globe.merge(ants_tmp)
            pherom_buffer = comm.recv(source=1, tag=1*100+3)

            pherom_globe.update_with(pherom_buffer)
            # food_counter = np.max(food_counter_list)


            # print(f"Process {rank} display")
    ...
    if rank == 0:
        ...
        with open("output_proc_update.txt", "w") as file:
            file.write(output)
