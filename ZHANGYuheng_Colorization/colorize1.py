...
def create_field( values, ifield, nb_layers, prolong_field = False ):
    ...
    de l'image.
    values : tableau de taille ny x nx x nfields
    ifield : indice du champs qu'on veut extraire
    nb_layers : nombre de couches de cellules fantomes
    """
    assert(ifield >= 0)
...
if __name__ == '__main__':
    ...
    values_gray_global = np.array(im_gray) 
    # On créer le tableau d'intensite en rajoutant deux couches de cellules fantomes dans chaque direction :
    total_height = values_gray_global.shape[0]
    # Calculer la hauteur de chaque tranche
    slice_height = total_height // size
    
    for i in range(size):
        if rank == i:
        # Calculer l'index de début et de fin pour chaque tranche
            start_idx = i * slice_height
            # Pour la dernière tranche, prendre tout jusqu'à la fin pour gérer les tailles non divisibles
            if i == size - 1:
                end_idx = total_height
            else:
                end_idx = start_idx + slice_height
            
            # Découper la tranche et l'ajouter à la liste
            values_gray = values_gray_global[start_idx:end_idx, :]
    ...
    val_ycbcr_global = np.array(im_ycbcr)
    ...
    for i in range(size):
        ...
        # Calculer l'index de début et de fin pour chaque tranche
            ...
            # Découper la tranche et l'ajouter à la liste
            val_ycbcr = val_ycbcr_global[start_idx:end_idx, :]
    ...
    val_hsv_global = np.array(im_hsv)
    for i in range(size):
        if rank == i:
        # Calculer l'index de début et de fin pour chaque tranche
            start_idx = i * slice_height
            ...
            # Découper la tranche et l'ajouter à la liste
            val_hsv = val_hsv_global[start_idx:end_idx, :]
    ...
    for i in range(size):
        if rank == i:
            if i != 0:
                comm.send(new_image_array, dest=0, tag=rank*10+1)
            elif i == 0:
                slices = [new_image_array]  
                for j in range(1, size):
                    slices.append(comm.recv(source=j, tag=j*10+1))
                new_image_array_global = np.concatenate(slices, axis=0)
