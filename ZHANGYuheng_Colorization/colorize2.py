...
def matrix_multiply(A, B, C, rank, size):
    """
    分块矩阵乘法，每个进程计算矩阵乘法的一部分。
    """
    # 假设A, B已经按适当的方式分块
    # C是乘法结果的容器，同样预先分配好
    # rank是当前进程的编号，size是总进程数
    # 这里的实现将依据实际情况有所不同
    
    # 示例：每个进程计算一个块的乘积
    # 注意：实际应用中应考虑矩阵尺寸与进程数的匹配和分块策略
    rows_per_proc = A.shape[0] // size
    start_row = rank * rows_per_proc
    end_row = (rank + 1) * rows_per_proc if rank != size - 1 else A.shape[0]
    C[start_row:end_row, :] = A[start_row:end_row, :].dot(B)
    return C

def minimize_parallel( A : sparse.csr_matrix, b : np.array, x0 : np.array, niters : int, epsilon : float, comm, rank : int, size : int):
    """
    Minimise la fonction quadratique a l'aide d'un gradient conjugue
    """
    Adotx0_loc = matrix_multiply(A, x0, np.zeros_like(b), rank, size)
    if rank == 0:
        Adotx0 = np.copy(Adotx0_loc)
        for i in range(1,size):
            Adotx0_loc = comm.recv(source=i, tag=i*10+1)
            Adotx0 += Adotx0_loc
    elif rank != 0:
        comm.send(Adotx0_loc, dest=0, tag=rank*10+1)
    ...
    Atransposedotr_loc = matrix_multiply(A.transpose(), r, np.zeros((A.transpose().shape[0], r.shape[1])), rank, size)
    if rank == 0:
        Atransposedotr = np.copy(Atransposedotr_loc)
        for i in range(1,size):
            Atransposedotr_loc = comm.recv(source=i, tag=i*10+2)
            Atransposedotr += Atransposedotr_loc
    elif rank != 0:
        comm.send(Atransposedotr_loc, dest=0, tag=rank*10+2)
    ...
    Adotp_loc = matrix_multiply(A, p, np.zeros((A.shape[0], p.shape[1])), rank, size)
    if rank == 0:
        Adotp = np.copy(Adotp_loc)
        for i in range(1,size):
            Adotp_loc = comm.recv(source=i, tag=i*10+3)
            Adotp += Adotp_loc
    elif rank != 0:
        comm.send(Adotp_loc, dest=0, tag=rank*10+3)
