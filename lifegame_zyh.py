...
if __name__ == '__main__':
    ...
    comm_dup = MPI.COMM_WORLD.Dup()  # On duplique le communicateur pour avoir deux sous-communicateurs
    rank_dup = comm_dup.rank # On récupère le rang du processus dans le communicateur
    size_couple = comm_dup.size # On récupère le nombre de processus dans le communicateur

    print(f"rank_couple={rank_dup}, size_couple={size_couple}")
    ...
    # elif size_couple == 2:
    #     if rank_dup == 0:  # Processus affichage
    #         grid = Grille(*init_pattern)
    #         appli = App((resx, resy), grid)
    #         while True:
    #             t1 = time.time()
    #             comm_dup.send(grid, dest=1, tag=1)
    #             cells = comm_dup.recv(source=1, tag=2)
    #             grid.cells = cells
    #             appli.draw()
    #             t2 = time.time()
    #             print(f"Temps affichage : {t2-t1:2.2e} secondes\r", end='')
    #     else:  # Processus calcul
    #         while True: 
    #             t3 = time.time()
    #             grid = comm_dup.recv(source=0, tag=1)
    #             grid.compute_next_iteration()
    #             comm_dup.send(grid.cells, dest=0, tag=2)
    #             t4 = time.time()
    #             print(f"Temps calcul prochaine generation : {t4-t3:2.2e} secondes\r", end='')
        ...
        while True:
            if rank_dup == 0:
                ...
                comm_dup.send(grid.cells, dest=0, tag=2)
                t4 = time.time()
            # print(f"Temps calcul prochaine generation : {t4-t3:2.2e} secondes, temps affichage : {t2-t1:2.2e} secondes\r", end='')


    # else :
    #     if rank_couple == 0:  # Processus calcul
    #         status_couple = 0
    #         grid = Grille(*init_pattern)
    #         appli = App((resx, resy), grid)
    #     else:  # Processus affichage
    #         status_couple = 1

    #     #initialisation des deux sous-communicateurs
    #     if rank_couple == 0:
    #         comm_couple.send(grid, dest=1, tag=0)
    #     elif rank_couple == 1:
    #         grid = comm_couple.recv(source=0, tag=0)
    ...
    #     size = comm.Get_size()
        ...
        #         rest_rows = dim[0] % size
        #         num_rows = dim[0] // size
        #         num_elements = num_rows * dim[1]
        #         send_counts = [num_rows * dim[1] for i in range(size)]
        #         send_counts[-1] += rest_rows * dim[1]

        #         displs = [sum(send_counts[:i]) for i in range(size)]

        #         rcv_counts = send_counts[rank]
        #         rcv_buffer = np.empty(rcv_counts, dtype=np.uint8)


        #         if rank == 0:
        #             send_buffer = grid.cells.flatten()
                    

        #         comm.Scatterv([send_buffer, send_counts, displs, MPI.UNSIGNED_CHAR], rcv_buffer, root=0)

        #         if rank == size - 1:
        #             rcv_buffer = rcv_buffer.reshape(int(send_counts[-1]/dim[1]), dim[1])
        #         else:
        #             rcv_buffer = rcv_buffer.reshape((num_rows, dim[1]))


        #         up_ghost = np.empty((1, dim[1]), dtype=np.uint8)
        #         down_ghost = np.empty((1, dim[1]), dtype=np.uint8)
        #         up_buffer = None
        #         down_buffer = None

        #         if rank == 0:
        #             up_ghost = rcv_buffer[0]
        #             down_ghost = rcv_buffer[-1]
        #             up_buffer = np.empty(up_ghost.shape, dtype=np.uint8)
        #             down_buffer = np.empty(down_ghost.shape, dtype=np.uint8)
        #             comm.Send(up_ghost, dest=size-1, tag=0)
        #             comm.Send(down_ghost, dest=1, tag=1)
        #             comm.Recv(up_buffer, source=size-1, tag=2*size-1)
        #             comm.Recv(down_buffer, source=1, tag=2)
        ...
        #             up_buffer = np.empty(up_ghost.shape, dtype=np.uint8)
        #             down_buffer = np.empty(down_ghost.shape, dtype=np.uint8)
        #             comm.Send(up_ghost, dest=rank-1, tag=2*rank)
        #             comm.Send(down_ghost, dest=0, tag=2*rank+1)
        #             comm.Recv(up_buffer, source=rank-1, tag=2*rank-1)
        #             comm.Recv(down_buffer, source=0, tag=0)
        #         else:
        #             up_ghost = rcv_buffer[0]
        #             down_ghost = rcv_buffer[-1]
        #             up_buffer = np.empty(up_ghost.shape, dtype=np.uint8)
        #             down_buffer = np.empty(down_ghost.shape, dtype=np.uint8)
        #             comm.Send(up_ghost, dest=rank-1, tag=2*rank)
        #             comm.Send(down_ghost, dest=rank+1, tag=2*rank+1)
        #             comm.Recv(up_buffer, source=rank-1, tag=2*rank-1)
        #             comm.Recv(down_buffer, source=rank+1, tag=2*rank+2)
                
        #         next_cells = rcv_buffer.copy()

        #         rcv_buffer = np.vstack((up_buffer, rcv_buffer))
        #         rcv_buffer = np.vstack((rcv_buffer, down_buffer))

        #         diff_cells = []
        #         ny = rcv_buffer.shape[0]
        #         nx = rcv_buffer.shape[1]
        ...
        #                 voisins_i = [i_above,i_above,i_above, i     , i      , i_below, i_below, i_below]
        #                 voisins_j = [j_left ,j      ,j_right, j_left, j_right, j_left , j      , j_right]
        #                 voisines = np.array(rcv_buffer[voisins_i,voisins_j])
        #                 nb_voisines_vivantes = np.sum(voisines)
        #                 if rcv_buffer[i,j] == 1: # Si la cellule est vivante
        #                     if (nb_voisines_vivantes < 2) or (nb_voisines_vivantes > 3):
        #                         next_cells[i-1,j-1] = 0 # Cas de sous ou sur population, la cellule meurt
        #                         diff_cells.append(num_elements*rank + (i-1)*dim[1]+j)
        #                     else:
        ...
        #                     diff_cells.append(num_elements*rank + (i-1)*dim[1]+j)
        #                 else:
        #                     next_cells[i-1,j-1] = 0         # Morte, elle reste morte.
                
        #         diff_glob_cells = comm.gather(diff_cells, root=0)

        #         if rank == 0:
        #             diff_glob_cells = functools.reduce(operator.iconcat, diff_glob_cells, [])
        #             comm_couple.send(diff_glob_cells, dest=0, tag=100)
                
            
        #     if status_couple == 0:
        #         diff_glob_cells = comm_couple.recv(source=0, tag=100)
        #         grid_cells = grid.cells.copy
        #         grid_shape = grid_cells.shape
        #         grid_cells = grid_cells.flatten()
        #         for i in diff_glob_cells:
        #             if grid_cells[i] == 0:
        #                 grid_cells[i] = 1
        #             else:
        #                 grid_cells[i] = 0
        #         grid.cells = grid_cells.reshape(grid_shape)

        #         comm_couple.send(grid, dest=1, tag=1000)
        #         appli.draw()

        #     if status_couple == 1:
        #         grid = comm_couple.recv(source=0, tag=1000)
        ...
        comm = comm_dup.Split(status_dup,rank_dup)
        rank = comm.Get_rank()
        size = comm.Get_size()

        if rank_dup==0:
            comm_dup.send(grid,dest=1,tag=0)
        elif rank_dup==1:
            grid=comm_dup.recv(source=0,tag=0)
        ...
        while True:
            if status_dup==1:
                ...
                num_rows = dim[0] // size
                num_elements = num_rows * dim[1]
                send_counts = [num_rows * dim[1] for _ in range(size)]
                send_counts[-1] += rest_rows * dim[1]
                ...
                if rank==0:
                    up_ghost=rcv_buffer[0]
                    down_ghost=rcv_buffer[rcv_buffer.shape[0]-1]
                    up_buffer=np.empty(up_ghost.shape,dtype=np.uint8)
                    down_bufffer=np.empty(down_ghost.shape,dtype=np.uint8)
                    comm.Send(up_ghost,dest=size-1,tag=0)
                    comm.Send(down_ghost,dest=1,tag=1)
                    comm.Recv(up_buffer,source=size-1,tag=2*size-1)
                    comm.Recv(down_bufffer,source=1,tag=2)
                elif rank==size-1:
                    up_ghost=rcv_buffer[0]
                    down_ghost=rcv_buffer[rcv_buffer.shape[0]-1]
                    up_buffer=np.empty(up_ghost.shape,dtype=np.uint8)
                    ...
                    comm.Send(down_ghost,dest=0,tag=2*rank+1)
                    comm.Recv(up_buffer,source=rank-1,tag=2*rank-1)
                    comm.Recv(down_bufffer,source=0,tag=0)
                else:
                    up_ghost=rcv_buffer[0]
                    down_ghost=rcv_buffer[rcv_buffer.shape[0]-1]
                    up_buffer=np.empty(up_ghost.shape,dtype=np.uint8)
                    ...
                    comm.Send(down_ghost,dest=rank+1,tag=2*rank+1)
                    comm.Recv(up_buffer,source=rank-1,tag=2*rank-1)
                    comm.Recv(down_bufffer,source=rank+1,tag=2*rank+2)

                next_cells=rcv_buffer.copy()
                
                rcv_buffer=np.vstack((up_buffer,rcv_buffer))
                rcv_buffer=np.vstack((rcv_buffer,down_bufffer))
                ...
                if rank==0:
                    diff_glob_cells=functools.reduce(operator.iconcat,diff_glob_cells,[])
                    comm_dup.send(diff_glob_cells,dest=0,tag=100)

            if status_dup==0:
                diff_glob_cells=comm_dup.recv(source=1,tag=100)
                grid_cells=grid.cells.copy()
                grid_shape=grid_cells.shape
                grid_cells=grid_cells.flatten()
                for ind in diff_glob_cells:
                    if grid_cells[ind]==1:
                        grid_cells[ind]=0
                    else:
                        grid_cells[ind]=1
                ...
                comm_dup.send(grid,dest=1,tag=1000)
                appli.draw()

            if rank_dup==1:
                grid=comm_dup.recv(source=0,tag=1000)
